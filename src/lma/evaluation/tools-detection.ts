...
export const evaluateToolsDetection = ({
    ...
    const expected = expectedOutput.useful_tools || [];
    const generated = generatedOutput.useful_tools || [];

    if (expected.length == 0 && generated.length == 0) return {
        toolNameIoU: null,
        toolParamAccuracy: null
    };
    ...
    for (const toolName of intersection) {
        ...
        if (expectedTool?.parameters && generatedTool?.parameters) {
            ...
            // If expected parameters are not specified, but the tool name matches,
            // treat parameters as non-constraining and therefore "correct".
            // This ensures tests that don't assert params don't get penalized.
            if (!expectedTool?.parameters && generatedTool?.parameters) {
                totalParamAccuracy += 1;
                totalParamSamples += 1;
            }
