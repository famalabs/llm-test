...
import { analyzeSentiment, SentimentAnalysisMode, SentimentScores } from "./sentiment-analysis";
...
const computeScores = (actual: Record<string, number>, predicted: SentimentScores) => {
...
const main = async () => {
    ...
    const data = JSON.parse(await readFile(input, "utf-8")) as Array<{ mode: SentimentAnalysisMode, input: string | string[]; scores: Record<string, number> }>;
    const output = [];

    for (const { input, scores, mode } of tqdm(data, { description: 'Analyzing sentiment', total: data.length })) {
