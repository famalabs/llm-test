...
export const USER_REQUEST_DETECTION_SCHEMA = () => z.object({ user_request: z.string().optional().nullable().describe("The detected user request as a string.") });
export const USER_SATISFACTION_DETECTION_SCHEMA = () => z.object({ request_satisfied: z.boolean().optional().nullable().describe("Indicates if the user request was satisfied.") });
export const USER_REQUEST_AND_TOOLS_DETECTION_SCHEMA = (includeToolsParams: boolean, includeRequestSatisfiedDetection: boolean) => {
    ...
    if (includeToolsParams) {
        return z.object({
            ...
            ...(includeRequestSatisfiedDetection ? { request_satisfied: z.boolean().optional().nullable().describe("Indicates if the user request was satisfied.") } : {})
        });
    }
