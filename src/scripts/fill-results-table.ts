...
const groupOrder = [
    'Battery', 'Pizza', 'Aspirina', 'Medicine'
]
...
function printTable() {
    ...
    // collect candidates.
    const allResults = Object.keys(data).map(llm => data[llm].results.map(test => ({...test, llm}))).flat();
    ...
    const candidateOrder = {}; // per group candidate order
    ...
    for (const group of groupOrder) {
        for (const candidate of candidateOrder[group]) { 
           ...
           table += [rouge, meteor, bleurt, bertscore, llm_judge_custom_small].join(separator);
